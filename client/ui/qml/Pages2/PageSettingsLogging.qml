...
PageType {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("CottonVPN logs")
        ...
        readonly property var exportLogsHandler: function() {
            var fileName = ""
            if (GC.isMobile()) {
                fileName = "CottonVPN.log"
            } else {
                fileName = SystemController.getFileName(qsTr("Save"),
                                                        qsTr("Logs files (*.log)"),
                                                        StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/CottonVPN",
                                                        true,
                                                        ".log")
            }
            if (fileName !== "") {
    ...
    QtObject {
        ...
        readonly property string description: qsTr("CottonVPN-service logs")
        ...
        readonly property var exportLogsHandler: function() {
            ...
            fileName = SystemController.getFileName(qsTr("Save"),
                                                    qsTr("Logs files (*.log)"),
                                                    StandardPaths.standardLocations(StandardPaths.DocumentsLocation) + "/CottonVPN-service",
                                                    true,
                                                    ".log")
            if (fileName !== "") {
                PageController.showBusyIndicator(true)
